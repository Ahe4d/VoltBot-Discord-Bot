...
client.on('message', async message => {
  ...
  if (
    ...
    if (pollentries[1].includes('second')) {
      timeset = convertTime(
        'seconds',
        parseInt(pollentries[1].match(/\d+/)[0])
      );
    ...
    if (timeset <= 604800000) {
      message.channel
        .send('**' + message.author + ' started a poll:** ' + pollquestion)
        .then(message => {
          ...
          message.awaitReactions(filter, { time: timeset }).then(collected => {
            thumbsupreact =
              message.reactions.find(reaction => reaction.emoji.name === `ðŸ‘`)
                .count - 1;
            thumbsdownreact =
              message.reactions.find(reaction => reaction.emoji.name === `ðŸ‘Ž`)
                .count - 1;
            message.delete();

            var result_firsthalf =
              'When asked **' +
              pollquestion +
              '**, people voted:\n' +
              thumbsupreact +
      ...
      message.channel.send(
        'Error: The time interval was too long. The maximum poll time allowed is 7 days.'
      );
    }
  }
  ...
  if (
    message.content === prefix + 'stream addchannel' &&
    message.author.id == config.owner
  ) {
    streamchannels.channel = message.channel.id;
    fs.writeFile(
      './data/streamchannels.json',
      JSON.stringify(streamchannels),
      function(err) {
  ...
  if (
    ...
    fs.writeFile(
      './data/streamchannels.json',
      JSON.stringify(streamchannels.enabled),
      function(err) {
        if (err) return console.log(err);
